...
private GroupEventController groupEventController;
    ...
    //Lista los grupos excepto en el que esta cada usuario
    public List<GameReunionDTO> listarGruposSinElUsuario(Integer userId) {
    return groupRepository.findAll().stream()
        .filter(group -> !participantRepository
            .existsByUserIdUserAndGroupIdGroup(userId, group.getIdGroup()))
        .map(group -> {
            int currentPlayers = participantRepository.countByGroupIdGroup(group.getIdGroup());
            return new GameReunionDTO(group, currentPlayers);
        })
